...
主な機能としては各会社の採用マイページへのリンク、ID/パスワードの一括管理と説明会や選考日程などのスケジュール管理です。
企業ごとのID/PWなどの機密情報を外部サーバーに送信せず、ブラウザ内データベース（IndexedDB）のみで完結させることで、プライバシーに配慮したセキュアな情報管理を実現しています。外部データベースを利用しないことで、コストをかけずに安定したデプロイを可能にしました。
データのバックアップも可能であり、機種変更やデータ消失にも対応できるよう実装しています。
